...
import { CalendarDays, Check, Edit3, ImageIcon, Plus, Search } from "lucide-react";
  ...
  async function save(submitEvent: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = kind === "products"
        ? { ...common, name: form.get("name"), slug: form.get("slug"), description: form.get("description"), priceLabel: form.get("priceLabel"), discountLabel: form.get("discountLabel") }
        : kind === "events"
...
export function AppointmentTable({ appointments }: { appointments: Appointment[] }) {
  ...
  const filtered = useMemo(() => appointments.filter((item) => `${item.name} ${item.email} ${item.phone} ${item.trainingInterest}`.toLowerCase().includes(query.toLowerCase())), [appointments, query]);
