...
export function AppointmentBlockManager({ blocks }: { blocks: AppointmentBlock[] }) {
  ...
  return <div className="schedule-manager"><form className="manager-form manager-form--embedded" onSubmit={create}><div className="admin-card__heading"><div><h2>Bloquear disponibilidad</h2><p>Cierra un día completo o únicamente un rango horario de una fecha concreta.</p></div></div><div className="block-mode" role="group" aria-label="Tipo de bloqueo"><button type="button" className={blockMode === "DAY" ? "active" : ""} aria-pressed={blockMode === "DAY"} onClick={() => setBlockMode("DAY")}><CalendarOff size={18} /><span><strong>Día completo</strong><small>No admite reservas ese día.</small></span></button><button type="button" className={blockMode === "RANGE" ? "active" : ""} aria-pressed={blockMode === "RANGE"} onClick={() => setBlockMode("RANGE")}><CalendarDays size={18} /><span><strong>Rango horario</strong><small>Cierra solo una parte del día.</small></span></button></div><div className={blockMode === "RANGE" ? "field-row field-row--three" : "field-row field-row--single-date"}><label>Fecha<input name="date" type="date" required /></label>{blockMode === "RANGE" && <><label>Desde<input name="startTime" type="time" step="3600" required /></label><label>Hasta<input name="endTime" type="time" step="3600" required /></label></>}</div><div className="field-row"><label>Aplicar a<select name="appointmentType" defaultValue="ALL"><option value="ALL">Todas las citas</option><option value="CONSULTATION">Solo consultas</option><option value="TRAINING">Solo entrenamientos</option></select></label><label>Motivo<input name="reason" defaultValue="Horario no disponible" maxLength={180} required /></label></div>{notice && <p className="form-error">{notice}</p>}<button className="button button--primary"><CalendarOff size={17} />{blockMode === "DAY" ? "Bloquear día" : "Bloquear rango"}</button></form><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Fecha y rango</th><th>Tipo</th><th>Motivo</th><th>Disponibilidad</th></tr></thead><tbody>{blocks.map((block) => { const fullDay = block.startTime === "00:00" && block.endTime === "23:59"; return <tr key={block.id}><td><strong>{block.date}</strong><span>{fullDay ? "Día completo" : `${block.startTime} – ${block.endTime}`}</span></td><td>{block.appointmentType === "ALL" ? "Todas" : block.appointmentType === "TRAINING" ? "Entrenamientos" : "Consultas"}</td><td>{block.reason}</td><td><button className={`button button--small ${block.active ? "button--outline" : "button--primary"}`} type="button" onClick={() => void toggle(block.id, !block.active)}>{block.active ? "Habilitar agenda" : "Volver a bloquear"}</button></td></tr>; })}</tbody></table>{!blocks.length && <div className="admin-empty"><CalendarOff /><h3>No hay turnos bloqueados.</h3><p>Agrega cierres, descansos o fechas sin atención.</p></div>}</div></div>;
}
