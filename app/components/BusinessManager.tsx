  ...
  async function save(submitEvent: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = kind === "products"
        ? { ...common, name: form.get("name"), slug: form.get("slug"), description: form.get("description"), priceLabel: form.get("priceLabel"), discountLabel: form.get("discountLabel"), dashboardContent: form.get("dashboardContent"), resourceUrl: form.get("resourceUrl"), checkoutProvider: form.get("checkoutProvider"), checkoutUrl: form.get("checkoutUrl"), checkoutExternalId: form.get("checkoutExternalId"), priceCents: Math.round(Number(form.get("price") || 0) * 100), currency: form.get("currency") }
        : kind === "events"
