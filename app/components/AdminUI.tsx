  ...
  return <><div className="manager-actions"><button className="button button--primary" type="button" onClick={openNew}><Plus size={17} />{kind === "trainings" ? "Nuevo entrenamiento" : "Nuevo artículo"}</button>{notice && <span role="status">{notice}</span>}</div>
    {creating && <form className="manager-form" onSubmit={save} key={editing?.id ?? "new"}><div className="admin-card__heading"><div><h2>{editing ? `Editar ${kind === "trainings" ? "entrenamiento" : "artículo"}` : kind === "posts" ? "Nuevo artículo" : "Nuevo entrenamiento"}</h2><p>Guarda cambios, adjunta recursos y controla la publicación desde la tabla.</p></div></div>
      {kind === "trainings" ? <>
        ...
        <div className="field-row"><label>Método de pago<select name="checkoutProvider" defaultValue={trainingEdit?.checkoutProvider || "MANUAL"}><option value="MANUAL">Coordinación manual</option><option value="STRIPE">Stripe Payment Link</option><option value="HOTMART">Hotmart</option></select></label><label>Enlace seguro de pago<input name="checkoutUrl" type="url" defaultValue={trainingEdit?.checkoutUrl || ""} placeholder="https://buy.stripe.com/... o https://pay.hotmart.com/..." /><small>El usuario deberá iniciar sesión antes de continuar.</small></label></div><div className="field-row"><label>Precio numérico<input name="price" type="number" min="0" step="0.01" defaultValue={(trainingEdit?.priceCents || 0) / 100} required /></label><label>Moneda<select name="currency" defaultValue={trainingEdit?.currency || "BOB"}><option>BOB</option><option>USD</option><option>EUR</option></select></label></div>
