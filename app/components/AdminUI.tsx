  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (kind === "trainings") {
        ...
        const payload = { name: data.get("name"), acronym: data.get("acronym"), slug: data.get("slug"), shortDescription: data.get("shortDescription"), fullDescription: data.get("fullDescription"), dashboardContent: data.get("dashboardContent"), checkoutProvider: data.get("checkoutProvider"), checkoutUrl: data.get("checkoutUrl"), checkoutExternalId: data.get("checkoutExternalId"), priceCents: Math.round(Number(data.get("price") || 0) * 100), currency: data.get("currency"), displayOrder: data.get("displayOrder"), logo: uploadedLogo || data.get("existingLogo") || "/logos/gdc-full-v2.jpg", heroImage: uploadedHero || data.get("existingHeroImage") || "", resourceUrl: uploadedResource || data.get("existingResourceUrl") || "" };
  ...
  return <><div className="manager-actions"><button className="button button--primary" type="button" onClick={openNew}><Plus size={17} />{kind === "trainings" ? "Nuevo entrenamiento" : "Nuevo artículo"}</button>{notice && <span role="status">{notice}</span>}</div>
    {creating && <form className="manager-form" onSubmit={save} key={editing?.id ?? "new"}><div className="admin-card__heading"><div><h2>{editing ? `Editar ${kind === "trainings" ? "entrenamiento" : "artículo"}` : kind === "posts" ? "Nuevo artículo" : "Nuevo entrenamiento"}</h2><p>Guarda cambios, adjunta recursos y controla la publicación desde la tabla.</p></div></div>
      {kind === "trainings" ? <>
        ...
        <label>Descripción corta<textarea name="shortDescription" rows={3} defaultValue={trainingEdit?.shortDescription} required /></label><label>Descripción completa<textarea name="fullDescription" rows={9} defaultValue={trainingEdit?.fullDescription} required /></label><label>Contenido para el panel del comprador<textarea name="dashboardContent" rows={5} defaultValue={trainingEdit?.dashboardContent || ""} placeholder="Instrucciones, bienvenida y próximos pasos que verá el usuario con acceso." /></label>
        <div className="field-row"><label>Método de pago<select name="checkoutProvider" defaultValue={trainingEdit?.checkoutProvider || "MANUAL"}><option value="MANUAL">Coordinación manual</option><option value="STRIPE">Stripe (precio o enlace)</option><option value="HOTMART">Hotmart</option></select></label><label>Enlace seguro de pago (opcional con price_ de Stripe)<input name="checkoutUrl" type="url" defaultValue={trainingEdit?.checkoutUrl || ""} placeholder="https://buy.stripe.com/... o https://pay.hotmart.com/..." /><small>El usuario deberá iniciar sesión antes de continuar.</small></label></div><div className="field-row"><label>ID de precio Stripe / producto Hotmart<input name="checkoutExternalId" pattern="[A-Za-z0-9_-]*" defaultValue={trainingEdit?.checkoutExternalId || ""} placeholder="price_… en Stripe / ID de producto en Hotmart" /><small>En Stripe usa el ID del precio que empieza por price_, no prod_. Con price_ el enlace es opcional. En Hotmart usa el ID del producto y su enlace de pago.</small></label><label>Precio numérico<input name="price" type="number" min="0" step="0.01" defaultValue={(trainingEdit?.priceCents || 0) / 100} required /></label></div><label>Moneda<select name="currency" defaultValue={trainingEdit?.currency || "BOB"}><option>BOB</option><option>USD</option><option>EUR</option></select></label>
