...
export function PublicShell({ children, whatsapp, notifications = [] }: { children: React.ReactNode; whatsapp?: string; notifications?: PublicNotification[] }) {
  ...
  return <WhatsAppProvider number={whatsapp}><SiteHeader notifications={notifications} /><main>{children}</main><SiteFooter /><WhatsAppFloat /><CookieConsent /></WhatsAppProvider>;
}
