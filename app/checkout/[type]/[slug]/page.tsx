...
export default async function CheckoutPage({ params }: { params: Promise<{ type: string; slug: string }> }) {
  ...
  return <main className="checkout-page"><a className="checkout-brand" href="/"><img src="/logos/gdc-full-v2.jpg" alt="Gimnasio del Cerebro" /><span>Pago seguro</span></a><section><a className="back-link" href={type === "producto" ? "/productos" : `/entrenamientos/${slug}`}><ArrowLeft size={16} />Volver</a><span className="login-kicker">Compra vinculada a {session.email}</span><h1>{item.name}</h1><p>{"description" in item ? item.description : item.shortDescription}</p><div className="checkout-summary"><div><span>Importe</span><strong>{item.priceCents > 0 ? new Intl.NumberFormat("es-BO", { style: "currency", currency: item.currency }).format(item.priceCents / 100) : "Por confirmar"}</strong></div><div><span>Método</span><strong>{provider === "MANUAL" ? "Coordinación con el equipo" : provider}</strong></div></div><div className="checkout-security"><LockKeyhole /><p>Gimnasio del Cerebro no almacena datos de tarjeta. El cobro se completa en el sitio seguro del proveedor configurado.</p></div>{provider !== "MANUAL" && item.checkoutUrl && item.priceCents > 0 ? <CheckoutButton itemType={type === "producto" ? "PRODUCT" : "TRAINING"} itemId={item.id} provider={provider} /> : <a className="button button--primary" href="/contacto"><ShieldCheck size={18} />Solicitar forma de pago</a>}</section></main>;
}
