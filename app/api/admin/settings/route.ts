...
export async function GET(request: Request) { if (!(await requestIsAdmin(request))) return Response.json({ error: "No autorizado" }, { status: 401 }); return Response.json({ settings: await getSettings() }); }
export async function PATCH(request: Request) { if (!(await requestIsAdmin(request))) return Response.json({ error: "No autorizado" }, { status: 401 }); const parsed = schema.safeParse(await request.json()); if (!parsed.success) return Response.json({ error: "Datos inválidos" }, { status: 400 }); await updateSettings(parsed.data); return Response.json({ ok: true }); }
