...
export async function GET(request: Request) { if (!(await requestIsAdmin(request))) return Response.json({ error: "No autorizado" }, { status: 401 }); return Response.json({ trainings: await getTrainings(true) }); }
