...
export async function POST(request: Request, context: { params: Promise<{ resource: string }> }) {
  ...
  if (resource === "products") {
    ...
    return Response.json({ id: await createProduct({ ...parsed.data, image: parsed.data.image || null, discountLabel: parsed.data.discountLabel || null, dashboardContent: parsed.data.dashboardContent || null, resourceUrl: parsed.data.resourceUrl || null, checkoutUrl: parsed.data.checkoutUrl || null, checkoutExternalId: parsed.data.checkoutExternalId || null }) }, { status: 201 });
  }
  if (resource === "events") {
