...
export async function PATCH(request: Request, context: { params: Promise<{ resource: string; id: string }> }) {
  ...
  if (resource === "products") {
    ...
    await updateProduct(id, { ...parsed.data, image: parsed.data.image || null, discountLabel: parsed.data.discountLabel || null, dashboardContent: parsed.data.dashboardContent || null, resourceUrl: parsed.data.resourceUrl || null, checkoutUrl: parsed.data.checkoutUrl || null });
