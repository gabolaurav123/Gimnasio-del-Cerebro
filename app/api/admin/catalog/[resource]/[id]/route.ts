...
const productSchema = z.object({ ...base, name: z.string().trim().min(2).max(150), slug: z.string().regex(/^[a-z0-9-]+$/), description: z.string().trim().min(10).max(3000), priceLabel: z.string().trim().min(1).max(80), discountLabel: z.string().trim().max(120).optional().default(""), dashboardContent: z.string().trim().max(10000).optional().default(""), resourceUrl: externalUrl.optional().default(""), checkoutProvider: z.enum(["STRIPE", "HOTMART", "MANUAL"]).default("MANUAL"), checkoutUrl: externalUrl.optional().default(""), checkoutExternalId: z.string().trim().max(120).regex(/^[A-Za-z0-9_-]*$/).optional().default(""), priceCents: z.coerce.number().int().min(0).max(1_000_000_000), currency: z.enum(["BOB", "USD", "EUR"]) });
...
export async function PATCH(request: Request, context: { params: Promise<{ resource: string; id: string }> }) {
  ...
  if (resource === "products") {
    ...
    await updateProduct(id, { ...parsed.data, image: parsed.data.image || null, discountLabel: parsed.data.discountLabel || null, dashboardContent: parsed.data.dashboardContent || null, resourceUrl: parsed.data.resourceUrl || null, checkoutUrl: parsed.data.checkoutUrl || null, checkoutExternalId: parsed.data.checkoutExternalId || null });
