...
const contentSchema = z.object({ title: z.string().trim().min(5).max(220), slug: z.string().regex(/^[a-z0-9-]+$/), excerpt: z.string().trim().min(10).max(500), content: z.string().trim().min(10).max(30000), category: z.string().trim().min(2).max(80), author: z.string().trim().max(120).optional().default(""), image: z.string().trim().max(500).refine((value) => !value || /^\/api\/media\/[a-z0-9-]+$/i.test(value)).optional().default("") });
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  else await updatePost(id, { ...parsed.data, image: parsed.data.image || null, author: parsed.data.author || null });
