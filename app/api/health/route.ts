...
export async function GET() {
  ...
  try {
    const runtime = await getRuntimeValues(["DATABASE_URL", "SITE_URL", "ADMIN_EMAIL", "ADMIN_PASSWORD", "ADMIN_PASSWORD_HASH", "SESSION_SECRET", "STRIPE_SECRET_KEY", "STRIPE_WEBHOOK_SECRET", "HOTMART_WEBHOOK_TOKEN", "PURCHASE_EMAIL_ENABLED", "RESEND_API_KEY", "PURCHASE_EMAIL_FROM"]);
    checks.environment = {
      ...
      stripeAutomationConfigured: Boolean(runtime.STRIPE_SECRET_KEY?.trim() && runtime.STRIPE_WEBHOOK_SECRET?.trim()),
      hotmartAutomationConfigured: Boolean(runtime.HOTMART_WEBHOOK_TOKEN?.trim()),
      purchaseEmailEnabled: /^(1|true|yes|on)$/i.test(runtime.PURCHASE_EMAIL_ENABLED?.trim() || ""),
      purchaseEmailConfigured: Boolean(runtime.RESEND_API_KEY?.trim() && runtime.PURCHASE_EMAIL_FROM?.trim()),
    };
  } catch (error) {
