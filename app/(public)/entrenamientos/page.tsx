...
export default async function TrainingsPage() {
  ...
  return <>
    ...
    <section className="program-list"><div className="shell">{trainings.map((training, index) => <article className="program-feature" id={training.slug} key={training.id}><div className="program-feature__logo"><span>{String(index + 1).padStart(2, "0")}</span><img src={training.logo} alt={`Logo oficial de ${training.name}`} width={700} height={700} loading="lazy" /></div><div className="program-feature__copy"><span className="program-feature__acronym">{training.acronym}</span><h2>{training.name}</h2><p>{training.shortDescription}</p><div className="button-row"><a className="button button--primary" href={`/entrenamientos/${training.slug}`}>Conocer el entrenamiento <ArrowRight size={18} /></a><a className="button button--outline" href={whatsappUrl(`Hola, quisiera recibir más información sobre ${training.name}.`, settings.whatsapp)} target="_blank" rel="noreferrer"><MessageCircle size={18} />Consultar</a></div></div></article>)}</div></section>
  </>;
}
