import { ArrowLeft, ArrowRight, CheckCircle2, Download, MessageCircle, ShoppingBag } from "lucide-react";
...
export default async function TrainingDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <section className="training-detail-hero"><div className="shell training-detail-hero__grid"><div><a className="back-link" href="/entrenamientos"><ArrowLeft size={16} />Todos los entrenamientos</a><span className="program-feature__acronym">{training.acronym}</span><h1>{training.name}</h1><p>{training.shortDescription}</p><div className="button-row"><a className="button button--light" href={whatsappUrl(`Hola, quiero adquirir el entrenamiento ${training.name}. ¿Podrían indicarme disponibilidad y forma de pago?`, settings.whatsapp)} target="_blank" rel="noreferrer"><ShoppingBag size={18} />Adquirir entrenamiento</a><a className="button button--ghost-light" href={whatsappUrl(`Hola, quisiera recibir más información sobre ${training.name}.`, settings.whatsapp)} target="_blank" rel="noreferrer"><MessageCircle size={18} />Consultar</a>{training.resourceUrl && <a className="button button--ghost-light" href={training.resourceUrl} target="_blank" rel="noreferrer"><Download size={18} />Descargar material</a>}</div></div><div className="training-detail-hero__logo"><img src={training.heroImage || training.logo} alt={`Imagen de ${training.name}`} width={720} height={720} /></div></div></section>
