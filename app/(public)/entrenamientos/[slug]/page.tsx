...
export default async function TrainingDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <section className="training-detail-hero"><div className="shell training-detail-hero__grid"><div><a className="back-link" href="/entrenamientos"><ArrowLeft size={16} />Todos los entrenamientos</a><span className="program-feature__acronym">{training.acronym}</span><h1>{training.name}</h1><p>{training.shortDescription}</p><a className="button button--light" href={whatsappUrl(`Hola, quisiera recibir más información sobre ${training.name}.`, settings.whatsapp)} target="_blank" rel="noreferrer"><MessageCircle size={18} />Consultar por WhatsApp</a></div><div className="training-detail-hero__logo"><img src={training.logo} alt={`Logo oficial de ${training.name}`} width={720} height={720} /></div></div></section>
