  ...
  return (
    <>
      <section className="hero">
        ...
        <div className="shell hero__content">
          <div className="hero__copy">
            ...
            <div className="button-row"><Link className="button button--light" href="/entrenamientos">Explorar entrenamientos <ArrowRight size={18} /></Link><a className="button button--ghost-light" href={whatsappUrl("Hola, quisiera recibir información sobre los entrenamientos de Gimnasio del Cerebro.")} target="_blank" rel="noreferrer"><MessageCircle size={18} />Hablar por WhatsApp</a></div>
      ...
      <section className="trainings-section" id="entrenamientos">
        <div className="shell"><div className="section-heading section-heading--split"><div><SectionEyebrow>Rutas de transformación</SectionEyebrow><h2>Nuestros entrenamientos</h2></div><div><p>Seis programas con enfoques distintos, conectados por una misma intención: comprender, entrenar y ampliar posibilidades.</p><Link className="text-link" href="/entrenamientos">Ver todos <ArrowRight size={16} /></Link></div></div><div className="training-grid">{trainings.map((training, index) => <TrainingCard training={training} index={index} key={training.id} />)}</div></div>
      </section>

      <section className="manifesto"><div className="shell manifesto__grid"><div className="manifesto__visual"><div className="manifesto__brain"><Image src="/logos/gdc-primary.jpg" alt="Isotipo oficial de Gimnasio del Cerebro" width={720} height={720} /></div><span className="orbit orbit--1" /><span className="orbit orbit--2" /></div><div className="manifesto__copy"><SectionEyebrow>Conocer para transformar</SectionEyebrow><h2>No se trata de saber más. Se trata de <em>comprender mejor.</em></h2><p>Gimnasio del Cerebro propone espacios de entrenamiento donde el conocimiento se vuelve experiencia, observación y nuevas formas de actuar.</p><div className="manifesto__points"><div><span>01</span><p><strong>Consciencia</strong>Observar cómo pensamos, sentimos y respondemos.</p></div><div><span>02</span><p><strong>Entrenamiento</strong>Llevar lo aprendido a la práctica cotidiana.</p></div><div><span>03</span><p><strong>Transformación</strong>Ampliar posibilidades desde una comprensión más profunda.</p></div></div><Link className="button button--primary" href="/contacto">Conocer Gimnasio del Cerebro <ArrowRight size={18} /></Link></div></div></section>

      <section className="journal-section"><div className="shell"><div className="section-heading section-heading--split"><div><SectionEyebrow>Ideas para seguir creciendo</SectionEyebrow><h2>Sigue entrenando tu mente</h2></div><div><p>Artículos, herramientas y contenido para seguir expandiendo tu consciencia.</p><Link className="text-link" href="/blog">Ver todos los artículos <ArrowRight size={16} /></Link></div></div><div className="blog-grid">{posts.slice(0, 3).map((post, index) => <BlogCard post={post} index={index} key={post.id} />)}</div></div></section>

      <section className="final-cta"><div className="shell final-cta__inner"><div><span className="final-cta__kicker">Tu próximo paso</span><h2>{settings.ctaTitle}</h2><p>{settings.ctaDescription}</p></div><div className="button-row"><Link className="button button--light" href="/entrenamientos">Explorar entrenamientos <ArrowRight size={18} /></Link><a className="button button--ghost-light" href={whatsappUrl("Hola, quisiera orientación para elegir un entrenamiento de Gimnasio del Cerebro.")} target="_blank" rel="noreferrer">Hablar por WhatsApp</a></div></div></section>
    </>
  );
}
